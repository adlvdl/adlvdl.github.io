...
In the previous post, I briefly checked the overlap of compound structures between the datasets, but barely looked at chemical structures or activity patterns.
In this post, we will get an overview of the chemical space across the four datasets and explore in detail the activity distribution.
...
On the other hand, it means some duplicated code as I wanted to keep the notebooks portable.
...
The first derived dataset contains all unique compounds across the single dose screen, dose response screen, counter screen and test dataset.
I make sure to normalize chemical structures using InChIKeys (somewhat against the advice of the OpenADMET organizers who recommend organizing data by Molecule Name or OCNT Batch ID) so each row represents one chemical structure. 
In this step I keep only those columns that I might potentially use for filtering and modeling in subsequent notebooks.
...
For the dose response and counter screen data I keep only the pEC50 and "Emax_estimate (log2FC vs. baseline)" columns.
...
In [notebook 1b](https://github.com/adlvdl/pxr_challenge/blob/main/marimo_notebooks/1b_chemical_space_and_mmp.py) ([html](../html_notebooks/pxr_challenge/1b_chemical_space_and_mmp.html)), I provide an overview of the chemical space. 
...
I decided to use an ECFP6 fingerprint as the basis for the t-SNE and UMAP plots.
...
The UMAP reveals a single, broadly dispersed cloud with no strong cluster separation.
The absence of tight, well-separated clusters suggests **high chemical diversity without dominant scaffold families**.
Dose-response compounds (light blue) and counter-screen compounds (dark blue) are distributed throughout the same space as the single-dose compounds (gray). 
...
How we define similarity can be very different. 
...
In datasets that are less diverse, I would be more strict when filtering MMPs, but the number of MMPs here is small enough even with this loose filter. 
...
Datasets with a large number of activity cliffs are called SAR discontinuous, and they are generally harder to model than datasets with low number of activity cliffs. 
...
Applying |ΔpEC50| ≥ 2 to the 1,503 MMP pairs within the dose-response set identifies **60 activity cliff pairs**, just 4% of MMP-connected compounds. 
...
![Pairwise similarity distributions for pairs of dose-response compounds](../images/posts/2026_04_15_pxr_sar_exploration/density_sim_fingerprints.png)
...
The number of ACs calculated from each fingerprint varies, but it generally hovers around 10% of similar pairs. 
However, while the number is similar the exact pairs identified are often different.

![Activity cliff overlap between MACCS and ECFP4 (Venn diagram)](../images/posts/2026_04_15_pxr_sar_exploration/venn_cliffs_maccs_ecfp4.png)
...
I want to test how distinct test compounds are from training compounds. 
...
All three are very similar, peaking near Tanimoto = 0.12–0.15.
**The test set seems indistinguishable from the training set in structural diversity and coverage.**
If the test set was structurally different from the train we would expect the cross-set similarity distribution to shift left.

The next analysis looks at the similarity value distribution between the training and test set to extract, for each test set compound, its nearest neighbor (NN) in the training set. 
...
As the final analysis I recreate a plot shown in the [example notebook](https://github.com/OpenADMET/PXR-Challenge-Tutorial) for the PXR activity submission, comparing dose response pEC50 to counter screen pEC50 values. 
...
This may reflect that the test set was assembled from a broader pool, focusing more on exploration than exploitation of highly active and selective compounds. 
...
This issue can often appear when you are trying retrospectively to assess decisions on compound sets using only the results. 
...
I was interested not in the topology of the nodes in a scaffold tree, but in the frequency of occurrence of Bemis-Murcko scaffolds, linked ring systems and individual ring systems, which together I call substructures for lack of a better word. 
I used this data to analyze and visualize the structural overlap between the training and test set. 
In this type of analysis, I am always interested to find large substructures (in terms of number of heavy atoms) that are present in a large number of compounds, as they can provide data-derived chemical series to cluster and classify the data. 
...
To conclude, let's go back to the questions I originally posed:
...
- What is the SAR character of the data (is it discontinuous)? I would say mildly discontinuous. We did find activity cliffs, but their propensity across similar pairs of compounds (12%-4%) is not massive. 
...
Next step will to be explore data splits, train baseline and single task models and generate my first submission.
That way I make sure to have at least some predictions ready for the mid challenge unblinding of part of the test set.
